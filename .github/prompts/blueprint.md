...
- If a `--- VISUAL REFERENCE ---` section appears at the end of this prompt, it contains extracted styles and a screenshot path from a URL the human explicitly tagged with a `[visual: <url>]` marker. Read the screenshot with the `Read` tool to see the actual visual appearance. Prefer these real values over guesses for Design Intent tokens. (Untagged URLs in the feature request carry no visual reference and should be treated as ordinary text.)
